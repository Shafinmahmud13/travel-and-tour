...
def orderBus(request, id):
    bus = get_object_or_404(Bus, id=id)

    order_item, created = OrderBus.objects.get_or_create(
        bus=bus,
        customer=request.user,
    )

    if created:
        messages.info(request, "This bus is added to your cart!")
        return redirect("blog:services")
    else:
        order_item.save()
        messages.info(request, "This bus is added to your cart.")
...
@login_required(login_url='blog:login')
def myBusOrders(request):
    orders = OrderBus.objects.filter(customer=request.user)
    context = {'orders': orders}
    return render(request, "admin/bus/bus_orders_admin.html", context)

@login_required
def orderBusDelete(request, id):
    order = get_object_or_404(OrderBus, id=id)

    if request.method == "POST":
        if request.POST.get('submit') == 'Yes':
            order.delete()
            messages.info(request, "The Bus Order Deleted Successfully!")
            return redirect("/myBusOrders")
        else:
            return redirect("/myBusOrders")

    return render(request, 'admin/bus/bus_order_delete.html')

def train_view(request):
    return render(request,"blog/train.html",)